...
<body class="page-create_diet_plan">
    ...
    <script>
        ...
        // Parse a free-text diet plan (ChatGPT output) into named sections + metadata.
        function parseDietText(raw) {
            const clean = s => s
                .replace(/\*\*/g, '')
                .replace(/`/g, '')
                .replace(/^[^\p{L}\p{N}]+/u, '')      // strip leading emoji / #, >, -, *, bullets
                .replace(/^\d+[.)]\s+/, '')            // "1. " / "2) " numbering (keeps "1.5")
                .replace(/\s*\|\s*/g, ' ')
                .trim();

            const SKIP = /^(choose(?: any)?(?: one| 1)?|pick one|for example|for eg|e\.?g\.?|use this(?: simple)? plate structure|plate structure|option\s*\w+|non-?vegetarian option|non-?veg option|vegetarian option|veg option)$/i;
            const AVOID_HEAD = /^(avoid|avoid these|avoid the following|things to avoid|foods to avoid|don'?t eat|do not eat|strictly avoid|no\b.*)$/i;
            const ADVICE = /^(try to |prefer |avoid |keep (?:dinner|lunch|breakfast|it|the)|don'?t |do not |note[:\s]|tip[:\s]|aim to |aim for |make sure|drink at least|stay hydrated|limit |reduce )/i;

            const SECTIONS = [
                { field: 'wake_up', keys: ['wake up', 'wake-up', 'wakeup', 'on waking', 'on rising', 'early morning', 'empty stomach', 'first thing', 'morning ritual', 'morning drink', 'after waking'] },
                { field: 'post_workout', keys: ['post workout', 'post-workout', 'after workout', 'intra workout', 'intra-workout', 'pre workout', 'pre-workout', 'workout meal', 'during workout'] },
                { field: 'breakfast', keys: ['breakfast', 'morning meal'] },
                { field: 'lunch', keys: ['lunch', 'afternoon meal', 'midday meal'] },
                { field: 'snack', keys: ['mid meal', 'mid-meal', 'midmeal', 'mid morning', 'mid-morning', 'mid-day snack', 'mid day snack', 'evening snack', 'evening', 'snack', 'snacks', 'brunch', 'tea time', 'tea-time', 'teatime'] },
                { field: 'dinner', keys: ['dinner', 'supper', 'evening meal', 'night meal'] },
                { field: 'pre_sleep', keys: ['pre sleep', 'pre-sleep', 'presleep', 'bed time', 'bedtime', 'before bed', 'before sleep', 'night cap', 'nightcap', 'post dinner', 'post-dinner'] },
                { field: 'guidelines', keys: ['guideline', 'guidelines', 'note', 'notes', 'instruction', 'instructions', 'tips', 'general tips', 'important', 'rules', 'do and don', 'dos and don', 'general guidelines', 'if hungry', 'if still hungry', 'if you feel hungry', 'late night', 'general advice', 'other tips'] }
            ];

            const headerField = (line) => {
                let l = clean(line).toLowerCase()
                    .replace(/\([^)]*\)/g, '')
                    .replace(/\s*:.*$/, '')
                    .replace(/\s+[—–]\s*.*$/, '')     // "... — 8:00 AM"
                    .replace(/\s+-\s+.*$/, '')         // "... - details"
                ...
                for (const s of SECTIONS) {
                    for (const k of s.keys) {
                        if (l === k || l.startsWith(k + ' ') || (l.startsWith(k) && l.length <= k.length + 6)) return s.field;
                    }
                }
                return null;
            };
            ...
            raw.split(/\r?\n/).forEach(rawLine => {
                ...
                if (f) {
                    ...
                    const noParen = line
                        .replace(/\([^)]*\)/g, '')
                        .replace(/\d{1,2}:\d{2}\s*(?:[–—-]\s*\d{1,2}:\d{2})?\s*(?:[ap]\.?m\.?)?/gi, '');
                ...
                if (current) {
                    ...
                    buckets[current].push(c);
                }
            });

            const out = { fields: {}, meta: {} };
            Object.keys(buckets).forEach(f => { if (buckets[f].length) out.fields[f] = buckets[f].join('\n'); });
            ...
            else if (/muscle (gain|build)|bulking|lean mass|hypertrophy|mass gain/i.test(raw)) out.meta.goal = 'Muscle Building';
